...
function createMobileControls() {
    ...
    this.fireButton = this.add.circle(screenWidth - 100, screenHeight - 100, 30, 0xff0000).setInteractive();
    this.fireButton.on('pointerdown', () => {
        controlKeys.FIRE.isDown = true;
    });
    this.fireButton.on('pointerup', () => {
        controlKeys.FIRE.isDown = false;
    });

    this.jumpButton = this.add.circle(screenWidth - 200, screenHeight - 100, 30, 0x00ff00).setInteractive();
    this.jumpButton.on('pointerdown', () => {
        controlKeys.JUMP.isDown = true;
    });
    this.jumpButton.on('pointerup', () => {
        controlKeys.JUMP.isDown = false;
    });
}

function dumpJoyStickState() {
    let cursorKeys = this.joyStick.createCursorKeys();
    this.joyStick.left = cursorKeys.left.isDown;
    this.joyStick.right = cursorKeys.right.isDown;
    this.joyStick.up = cursorKeys.up.isDown;
    this.joyStick.down = cursorKeys.down.isDown;
}
